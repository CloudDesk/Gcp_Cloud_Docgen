...
console.log('inside convertDocxBufferToPdf');
  try {
  let writefile =   await fs.writeFile(tempDocxPath, docxBuffer);
console.log(writefile, "write file data");
    await execAsync(
 ...
 */
export const generatePdfsFromTemplate = async (
  templatePath: string,
  fieldsDataArray: any[],
  baseFileName: string
...
console.log(outputDir, "output directory path");
    ...
    for (let i = 0; i < fieldsDataArray.length; i++) {
      ...
      const pdfRelativePath = await convertDocxBufferToPdf(
        docxBuffer,
        outputDir,
